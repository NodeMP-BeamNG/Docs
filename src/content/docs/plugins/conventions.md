...
| Client `node` table | client mod `1.6.10` | The ten functions on [Client scripting](/plugins/client-scripting/). |
| `NodeMP.*` | client mod `1.6.10` (`NodeMP.VERSION`) | One stable global; the original flat helpers stay as aliases of the namespaced calls. `NodeMP.internal` and the dotted module names underneath may move between versions. |
