...
| C ABI | `2.2` (`NODE_ABI_VERSION_MAJOR` `2`, `MINOR` `2`) | The major is the layout: nothing moves within it, new entries are appended and bump the minor, a retired entry becomes a stub that keeps its slot. A module built against an older `2.x` keeps working; a different major is refused by the loader -- major 2 is the first such break (the `pg_*` entries left with the database), so a module built against a `1.x` header must be rebuilt. |
