...
| `ServerConfig.toml`, `[General]` with `AuthKey` | `server.toml`, seven sections: `[General]`, `[Resources]`, `[Content]`, `[Network]`, `[Experimental]`, `[Directory]`, `[Http]` - see [Configuration](/hosting/configuration/); the server key is `[Directory] HostId` and `HostSecret` |
