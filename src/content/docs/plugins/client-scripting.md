...
- **Replacing the game's or the client mod's files.** Not possible. A client file is never written
  ...
  (`nametags` and `damage` are the modules that register with it in client mod 1.6.8;
...
error. `NodeMP.VERSION` is the mod version, `1.6.8`. Client files can call the same table; the
...
Client mod 1.6.8 can run a player's session under **strict** rules - the client half of what
