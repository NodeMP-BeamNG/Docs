...
- Новые возможности добавляются в конец и поднимают минор. Ничто не переставляется и не удаляется;
  выведенный из употребления вызов становится заглушкой, сохраняющей своё место. Любое изменение,
  которое сдвигает поле, поднимает мажор.
- Первые два члена - простые поля для вашей собственной проверки: `abi_version` (индекс 0) - это
  ...
  ему вызовов групп видимости (`dimensions: this server has no visibility groups (needs ABI 1.7)`).
...
| Зарегистрированные колбэки: клиентские события, встроенные события, вердикты, уведомления о машинах, таймеры, `done` задания, ответы HTTP, сообщения шины, данные модульного канала, `load`/`unload` языкового хоста | единственном рабочем потоке фреймворка, по одному за раз | Никогда два одновременно и никогда параллельно с Lua-обработчиком. Блокировка здесь останавливает каждый ресурс: держите колбэки короткими, а долгую работу отдавайте `submit_job` или собственному потоку. |
...
имени. Оба выполняются в рабочем потоке, по одному за раз. Клиентские файлы хостируемого ресурса
упаковываются точно так же, как у Lua-ресурса: клиент выполняет Lua игры, на каком бы языке ни была
написана серверная половина. `register_language_host` возвращает `-1` при `struct_size`, не
