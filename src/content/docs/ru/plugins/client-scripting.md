...
- **Нет автономного запуска.** Клиентские файлы выполняются только внутри игры: сервер упаковывает
  их, не выполняя (начиная с сервера 1.2.1 он проверяет их синтаксис и пишет `Error` для файла,
  который не разбирается), и ничто на стороне сервера не может вызвать обработчик `node.on`
