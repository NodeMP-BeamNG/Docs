...
По сервер 1.2.1 включительно это был `node.pg` в ядре. Больше нет - движок, его клиентская
...
|---|---|
| `nil` / `db.NULL` | `NULL` (сентинел нужен внутри массива параметров, где `nil` обрывает массив) |
| `boolean` | `true`/`false` |
...
- **NULL.** `{ 1, nil, "x" }` отправляет *один* параметр - `nil` обрывает массив, - и оператор тогда
  падает из-за нехватки `$2`. Пишите `{ 1, db.NULL, "x" }`. В обратную сторону ячейка `NULL` просто
