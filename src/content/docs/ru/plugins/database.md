...
`node.pg` даёт ресурсу асинхронный доступ к PostgreSQL хоста: параметризованные операторы,
...
возвращается колбэком или возобновляет вашу корутину. Хост задаёт одну строку подключения в
...
|---|---|---|
| Где | JSON-файл на ресурс в `storage/`, целиком в памяти | PostgreSQL хоста |
| Настройка | не нужна | хост устанавливает PostgreSQL и задаёт `[Database] Url` |
...
Эта часть - для хоста. Серверу нужны роль PostgreSQL, база данных, в которой он может создавать
...
- **NULL.** `{ 1, nil, "x" }` отправляет *один* параметр - `nil` обрывает массив, - и оператор с
  `$2` в нём тогда падает с SQLSTATE `08P01` (Postgres: `bind message supplies 1
  parameters, but …`). Пишите `{ 1, node.pg.NULL, "x" }`. В обратную сторону ячейка `NULL` просто
...
|---|---|---|
| `pg_disabled` | `[Database] Url` пуст; драйвер выключен. | Проверьте `node.pg.enabled()` при загрузке и работайте без этой возможности или скажите хосту. |
...
| `08P01` | Нарушение протокола, на практике - число параметров, не совпадающее с оператором: `{ 1, nil, 2 }` отправляет один параметр, потому что `nil` обрывает массив. | `node.pg.NULL` для пустого места. |
| `pg_params` | Значение, которое драйвер не может отправить, - функция, корутина, userdata кроме `node.pg.NULL`, строка с `NUL`, таблица, которую JSON не кодирует, - или больше 1000 параметров. Сообщение - лаконичное `query not queued`; смотреть надо в таблицу параметров. | Исправьте таблицу параметров. |
