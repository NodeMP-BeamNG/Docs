...
Вам нужны работающий `Node-Server` 1.5.0 ([Быстрый старт](/ru/hosting/quick-start/)), открытый в
...
`hello` (скопируйте `examples/chat` из архива релиза в `resources/`; `examples/README.txt`
перечисляет остальные) или знайте, что без него эти два вызова не делают вообще ничего - ни строки
в чате, ни строки в консоли. В консоли видно `node.log`.
...
таймеры, `node.storage`, модуль `db`, `node.http`, перезагрузка, `serverShutdown` и `resourceUnload`
...
игры не запустить вовсе: сервер упаковывает файлы и проверяет их синтаксис, но не выполняет их, и
первое место, где видна ошибка времени выполнения, - `beamng.log` игрока.
...
каждый обработчик, колбэк таймера, отрезок корутины и колбэк завершения измеряется отдельно, а
строка называет ресурс и то, что зависло (до 1.2.1 измерялись только целые задания рабочего потока -
медленный колбэк таймера или отрезок `node.async` оставался незамеченным, - и ресурс не назывался);
см. [Конкурентность](/ru/plugins/concurrency/).
...
не выполняет клиентские файлы, но разбирает их при упаковке: файл, который не разбирается,
сообщается при загрузке строкой
...
независимо от настройки обфускации (до 1.2.1 единственным признаком было предупреждение
Prometheus, а при выключенной обфускации его не было вовсе). Где лежит лог и как открыть
внутриигровую консоль диагностики - на странице
