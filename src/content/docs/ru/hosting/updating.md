...
печатает `Node-Server v1.3.0`. Стартовый баннер показывает то же число в строке `version`, а
...
два архива, названных по версии (`Node-Server-1.3.0-linux-x64.tar.gz`,
`Node-Server-1.3.0-windows-x64.zip`) и их файлы `.sha256`, а его описание содержит заметки о
...
(`ghcr.io/nodemp-beamng/server:v1.3.0`); `latest` следует за новейшей сборкой ветки main и может
...
Лаунчер и сервер говорят на версионированном сетевом протоколе: **`v21` начиная с сервера 1.3.0**,
который идёт вместе с лаунчером 1.1.9 и клиентским модом 1.5.4. 1.1.0 подняла его с `v17` до `v18`
...
машины и настройка «кто может сесть» как кадры), поэтому сервер версии 1.2.1 и лаунчер 1.1.9 отказывают
...
(1.1.1, 1.2.0, 1.2.1), не требует от ваших игроков обновлять лаунчер; тот, что двигает его (1.3.0), — требует.
