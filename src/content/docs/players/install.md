...
description: Download and install the NodeMP launcher 1.1.11 on Windows, let it install the client mod, and sign in or continue as Test Drive.
---
...
- BeamNG.drive **0.39.4.0**, the current release: client mod 1.6.0 is built for it, and a strict
...
1. Download `NodeMP-Setup-1.1.11.exe` from [nodemp.com/download](https://nodemp.com/download)
   (the *Launcher (Windows)* card). The same file is attached to tag `launcher-v1.1.11` at
...
2. The installer is not code-signed yet, so Windows SmartScreen may warn about an unknown
   ...
   published one: `Get-FileHash .\NodeMP-Setup-1.1.11.exe` in PowerShell prints the SHA-256 of the
