...
- BeamNG.drive **0.39.4.0**, the current release: client mod 1.4.0 is built for it, and a strict
...
2. The installer is not code-signed yet, so Windows SmartScreen may warn about an unknown
   ...
   published one: `Get-FileHash .\NodeMP-Setup-1.1.0.exe` in PowerShell prints the SHA-256 of the
