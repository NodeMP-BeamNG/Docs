...
description: Download and install the NodeMP launcher 1.1.7 on Windows, let it install the client mod, and sign in or continue as Test Drive.
---
...
- BeamNG.drive **0.39.4.0**, the current release: client mod 1.5.2 is built for it, and a strict
...
1. Download `NodeMP-Setup-1.1.7.exe` from [nodemp.com/download](https://nodemp.com/download)
   (the *Launcher (Windows)* card). The same file is attached to tag `launcher-v1.1.7` at
...
2. The installer is not code-signed yet, so Windows SmartScreen may warn about an unknown
   ...
   published one: `Get-FileHash .\NodeMP-Setup-1.1.7.exe` in PowerShell prints the SHA-256 of the
