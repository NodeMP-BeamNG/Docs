...
| `Could not join · This server needs a newer launcher — update from the Download page` | The server speaks a newer wire protocol than this launcher. The server's reason is `Protocol version mismatch: launcher speaks v17, server speaks v21 - update the outdated side`, with the live numbers; launcher 1.1.9 speaks v21. | Install the current launcher from [nodemp.com/download](https://nodemp.com/download); the panel under the server's card has an *Open the Download page* button. |
| `Could not join · This server runs an older NodeMP server (protocol v18; this launcher speaks v21)` | The same mismatch the other way round: the server is behind the launcher. | Nothing on your side; the host has to update the server. |
