...
| `Could not join · This server needs a newer launcher — update from the Download page` | The server speaks a newer wire protocol than this launcher. The server's reason is `Protocol version mismatch: launcher speaks v17, server speaks v22 - update the outdated side`, with the live numbers; launcher 1.1.10 speaks v22. | Install the current launcher from [nodemp.com/download](https://nodemp.com/download); the panel under the server's card has an *Open the Download page* button. |
