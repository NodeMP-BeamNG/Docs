...
| Configuration | `server.toml` with `[General]`, `[Resources]`, `[Content]`, `[Network]`, `[Experimental]` and `[Directory]`; every key also has a `NODE_*` environment variable. | `ServerConfig.toml` with a `[General]` section. |
