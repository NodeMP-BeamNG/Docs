...
| Configuration | `server.toml` with seven sections - `[General]`, `[Resources]`, `[Content]`, `[Network]`, `[Experimental]`, `[Directory]` and `[Database]`; every key also has a `NODE_*` environment variable (see [Configuration](/hosting/configuration/)). | `ServerConfig.toml` with a `[General]` section. |
...
| Transport | TLS 1.3 over TCP between the helper and the server, UDP for the position stream with an authentication trailer; every packet is a typed `(Category, SubType)` frame, wire protocol v17. The server refuses a launcher with another version. | Plain TCP and UDP with text-tagged packets. |
