...
| | NodeMP | BeamMP |
|---|---|---|
| Transport | TLS 1.3 on the TCP session; UDP for the position stream with an authentication trailer; every packet a typed binary `(Category, SubType)` frame. Wire protocol v18 is checked at connect and a launcher with another version is refused. | Plain TCP and UDP; packets tagged by a leading character; game traffic is not encrypted. |
